...
func TestFindTaskGroups(t *testing.T) {
	t.Run("Should find task groups correctly", func(t *testing.T) {
		g := []*graphNode{
			{
				task:  dummyTask("5"),
				edges: []string{"11"},
		...
		result, err := findTaskGroups(g)
		if err != nil {
			t.Fatalf("expected no error, but got %s", err)
		}

		if fmt.Sprint(result) != "[[9 10 2] [11 8] [3 7 5]]" {
			t.Fatalf("expected [2 9 10 8 11 3 5 7], but got %s", result)
		}
	})
}
